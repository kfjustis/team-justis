...
eval (Declare decls body) env = eval body newEnv
  where vars        = map fst decls
        ...
        values      = map (eval ((expressions)!!(n-1))) [env]   --choosing the n-1th element of the expressions list 
...
{- --(execute $ parseExp ("var x = 5;" ++ "var f = function(y) {var y = x * y; function(x) {x + y} };" ++ "var g = f(2);" ++"g(5)")) == (IntV 15)
